...
**Claude Desktop note:** in Desktop the agent usually cannot write files. If you cannot create/write `~/.config/jira-tools/config.json`, do NOT fall back to "add a folder" or "download the file" — instead tell the user that in Claude Desktop the Jira URL and token are set directly in the **plugin's config panel** (filled at install / in the plugin settings), and stop. This file-based flow is for Claude Code.
...
4. **Verify the connection** by calling the `get_current_user` MCP tool (server `jira`). On success show its output ("Logged in as …"). The server re-reads the config on every call, so no reload is needed.

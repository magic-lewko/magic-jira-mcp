...
- All tools (read + write: `create_issue`, `create_issues` (bulk, max 50 per call), `update_issue`, `add_comment`, `add_attachment`, `transition_issue`, `assign_to_epic`, `link_issues`) are always registered — there is NO write-mode/`allowWrite` gate. Server-side write rails (code, not skill instructions) guard against AI mistakes, not user intent: explicit batches not loops (`create_issue` = one issue; `create_issues` = up to 50 in one request, all-or-nothing against the budget), session write budget (default 100 creates / 300 writes, `writeBudget` in config or `JIRA_WRITE_BUDGET_*` env; hard stop against loops), duplicate guard on creates (scoped to the parent for sub-tasks; `allow_duplicate=true` only after explicit user confirmation), and `/create-task`'s mandatory dry-run. AI transparency is always on: created issues get an `ai-generated` label and comments a signature suffix — enforced in code.

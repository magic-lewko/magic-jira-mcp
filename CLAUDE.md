...
- Write tools (`create_issue`, `add_comment`, `transition_issue`) are registered ONLY when `JIRA_ALLOW_WRITE=true`. Default is read-only. Server-side write rails (code, not skill instructions): one issue per call, session write budget (default 10 creates / 30 writes, `writeBudget` in config or `JIRA_WRITE_BUDGET_*` env), duplicate guard on `create_issue` (`allow_duplicate=true` only after explicit user confirmation).
